...
$(()=>{
   ...
   $.ajax({
       url:add_blood_process_link,
       method:"POST",
       data: $("#form_add_blood").serialize(),
       dataType:"json",
       ...
       error: function(response) {
        ...
        $('#blood_group_name_error').text(response.responseJSON.errors.blood_group_name);
...
table_cot = $('#blood_group_data').DataTable({
    ...
    createdRow: function( row, data, dataIndex ) {
        ...
        $( row ).find('td:eq(1)').attr('data-label', 'Blood Group Name');
...
function blood_edit_process(id){

    $('#blood_edit_pop_modal_div').modal('show');

    $.ajax({
        url: get_blood_details_link,
        method: "POST",
        data:{"id":id,},
        dataType: "json",
        success: function(data) {
            ...
            if(data.length !=0){
                $('#blood_group_name').val(data[0].blood_group_name);
                $('#ed_id').val(id);
            }
        }
    });
...
$("#editUpdate").on('click', function() {
    ...
    $.ajax({
        url: update_blood_details_link,
        method: "POST",
        data:{
            "id":ed_id,
            "blood_group_name":ed_blood_group_name,
        },
        dataType: "json",
        success: function(data) {
...
function blood_status_process(id, status_data){
    // $('#confirmbox').click();
    $('#blood_pop_modal_div').modal('show');

    $("#confirmSubmit").on('click', function() {
        $('#close_status_pop').click();

        $.ajax({
            url: process_blood_status_link,
            method: "POST",
...
function blood_delete_process(id){
    // $('#confirmbox').click();
