...
$(()=>{
   ...
   $.ajax({
       url:add_roll_process_link,
       method:"POST",
       data: $("#form_add_roll").serialize(),
       dataType:"json",
       ...
       error: function(response) {
        ...
        $('#roll_name_error').text(response.responseJSON.errors.roll_name);
...
function roll_edit_process(id){

    $('#roll_edit_pop_modal_div').modal('show');

    $.ajax({
        url: get_roll_details_link,
        method: "POST",
        data:{"id":id,},
        dataType: "json",
        success: function(data) {
            ...
            if(data.length !=0){
                $('#roll_name').val(data[0].roll_name);
                $('#ed_id').val(id);
            }
        }
    });
...
$("#editUpdate").on('click', function() {
    // alert("abc");
    var ed_roll_name = $('#roll_name').val();
    var ed_id = $('#ed_id').val();

    $.ajax({
        url: update_roll_details_link,
        method: "POST",
        data:{
            "id":ed_id,
            "roll_name":ed_roll_name,
        },
        dataType: "json",
        success: function(data) {
        ...
        error: function(response) {
            ...
            $('#edit_roll_name_error').text(response.responseJSON.errors.roll_name);
...
function roll_status_process(id, status_data){
    // $('#confirmbox').click();
...
function roll_delete_process(id){
    // $('#confirmbox').click();
