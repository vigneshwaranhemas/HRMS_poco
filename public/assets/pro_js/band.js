...
$(()=>{
   ...
   $.ajax({
       ...
       error: function(response) {
        ...
        $('#band_name_error').text(response.responseJSON.errors.band_name);
...
table_cot = $('#band_data').DataTable({
    ...
    createdRow: function( row, data, dataIndex ) {
        ...
        $( row ).find('td:eq(1)').attr('data-label', 'Band Name');
...
function band_edit_process(id){

    $('#band_edit_pop_modal_div').modal('show');
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if(data.length !=0){
                $('#band_name').val(data[0].band_name);
                $('#ed_id').val(id);
            }
        }
    });
...
$("#editUpdate").on('click', function() {
    // alert("abc");
    var ed_band_name = $('#band_name').val();
    var ed_id = $('#ed_id').val();

    $.ajax({
        url: update_band_details_link,
        method: "POST",
        data:{
            "id":ed_id,
            "band_name":ed_band_name,
        },
        dataType: "json",
        success: function(data) {
        ...
        error: function(response) {
            ...
            $('#edit_band_name_error').text(response.responseJSON.errors.band_name);
